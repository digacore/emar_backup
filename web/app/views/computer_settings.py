...
computer_settings_blueprint = Blueprint("computer_settings", __name__, url_prefix="/computer_settings")
...
@computer_settings_blueprint.route("/update_printer_info/<int:computer_id>", methods=["POST"])
@login_required
def update_printer_info(computer_id: int):
    ...
    if telemetry_settings.send_printer_info != status:
        ...
        if linked_table:
            ...
            # create link table
            telemetry_settings = m.TelemetrySettings(
                send_printer_info=status
            )
            telemetry_settings.save()
            new_link_table = m.ComputerSettingsLinkTable(
                computer_id=computer.id,
                telemetry_settings_id=telemetry_settings.id
            )
            new_link_table.save()
